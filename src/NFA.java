...
public class NFA {
	...
	public DFAHolder toDFA() {
		initializeStates();
		Map<Map<Set<Integer>, Character>, Integer> seq = new HashMap<>();
		Map<Map<Set<Integer>, Character>, Set<Integer>> rst = new HashMap<>();
		Deque<Set<Integer>> stack = new LinkedList<>();
		stack.push(initStates);
		while (!stack.isEmpty()) {
			Set<Integer> ret = stack.pop();
			System.out.println(rst.toString());
			for (char c : inputChars) {
				Set<Integer> states = calClosure(move(ret, c));
				if (!stack.contains(states)) stack.push(states);
